...
public class AudioDeviceMonitor {
  ...
  /// Notification posted when audio devices change on macOS
  public static let audioDeviceChangedNotification = Notification.Name("AudioDeviceChanged")
  
  /// Sets up monitoring for audio device changes on macOS
  public static func startMonitoring() {
#if os(macOS)
    setupCoreAudioDeviceMonitoring()
#endif
  }
  ...
  public static func stopMonitoring() {
    ...
    AudioObjectRemovePropertyListener(AudioObjectID(kAudioObjectSystemObject), &address, audioDevicePropertyListener, nil)
    
    // Also remove output device listener
...
#if os(macOS)
  /// Sets up Core Audio monitoring for device changes
  private static func setupCoreAudioDeviceMonitoring() {
    var address = AudioObjectPropertyAddress(
      mSelector: kAudioHardwarePropertyDefaultInputDevice,
      mScope: kAudioObjectPropertyScopeGlobal,
      mElement: kAudioObjectPropertyElementMain
    ...
    AudioObjectAddPropertyListener(AudioObjectID(kAudioObjectSystemObject), &address, audioDevicePropertyListener, nil)
    
    // Also monitor output device changes
    address.mSelector = kAudioHardwarePropertyDefaultOutputDevice
    AudioObjectAddPropertyListener(AudioObjectID(kAudioObjectSystemObject), &address, audioDevicePropertyListener, nil)
  }
...
#if os(macOS)
/// Global callback function for Core Audio property listener
/// This needs to be a global function to work with C function pointers
private func audioDevicePropertyListener(
  ...
  DispatchQueue.main.async {
    NotificationCenter.default.post(
      name: AudioDeviceMonitor.audioDeviceChangedNotification,
      object: nil
    )
  }
  return noErr
